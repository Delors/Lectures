...
class Quizzy extends HTMLElement {
    ...
    #ws = null; // will be initialized once we know if we are a client or admin
    ...
    connectedCallback() {
         ...
         * Client logic
         */
        this.shadowRoot.getElementById('clientButton').addEventListener('click', () => {
            ...
            const ws = this.#ws = new WebSocket(`${wsURL}/player`);
            ...
            const submitUsername = () => {
                const name = this.shadowRoot.getElementById("username").value;
                ws.send(JSON.stringify({
                    "type": "registration",
                    "name": name
                }));
         ...
         * Admin logic
         */
        this.shadowRoot.getElementById('adminButton').addEventListener('click', () => {
            this.shadowRoot.innerHTML = `
                <main>
                    <h1>Players</h1>
                    <p id="players"></p>
            ...
            ws.onmessage = (event) => {
                ...
                switch (data.type) {
                    case "players":
                        const players = this.shadowRoot.getElementById("players")
                        players.innerText =
